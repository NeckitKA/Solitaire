...
void TForm1::ChangeCardsStacksByRightClick() {
	bool nСMFW = false; //noСardsMoveFromWaste
	bool nСMFT0 = false; //noСardsMoveFromTableau0
	bool nСMFT1 = false;
	bool nСMFT2 = false;
	bool nСMFT3 = false;
	bool nСMFT4 = false;
	...
	while (!AVP) {
		...
		if (!tableau0->cards.empty()) {
			while (!tableau0->cards.empty() && !nСMFT0) {
		...
		if (!tableau1->cards.empty()) {
			while (!tableau1->cards.empty() && !nСMFT1) {
		...
		if (!tableau2->cards.empty()) {
			while (!tableau2->cards.empty() && !nСMFT2) {
		...
		if (!tableau3->cards.empty()) {
			while (!tableau3->cards.empty() && !nСMFT3) {
		...
		if (!tableau4->cards.empty()) {
			while (!tableau4->cards.empty() && !nСMFT4) {
		...
		if (!tableau5->cards.empty()) {
			while (!tableau5->cards.empty() && !nСMFT5) {
		...
		if (!tableau6->cards.empty()) {
			while (!tableau6->cards.empty() && !nСMFT6) {
		...
		if (nСMFW && nСMFT0 && nСMFT1 && nСMFT2 && nСMFT3 && nСMFT4 && nСMFT5 && nСMFT6) {
			nСMFW = false;
			nСMFT0 = false;
			nСMFT1 = false;
			nСMFT2 = false;
